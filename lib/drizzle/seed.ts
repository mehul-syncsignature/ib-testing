...
import { plans, planLimits } from "./schema";
...
async function main() {
  ...
  for (const planLimit of planLimitData) {
    await serverDb
      .insert(planLimits)
      .values({
        ...planLimit,
      ...
      .onConflictDoUpdate({
        target: planLimits.id,
        set: {
          allowedTemplates: planLimit.allowedTemplates,
          updatedAt: new Date(),
        },
      });
  }
